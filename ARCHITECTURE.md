...
- **Post-succession phase:** When Khamenei dies (observed or simulated), the simulation enters a non-terminal succession state (SUCCESSION_CONTESTED or SUCCESSION_CONSOLIDATING) with mourning dynamics (nationalist_rally / revolutionary_surge / paralysis), elite cohesion decay, economic shock feedback loops, and condition-gated consolidation. Consolidation requires: mourning over, cohesion > 0.5, protests < 10%, no defection. Intel claims tagged `elite_fracture`/`irgc_split` seed initial cohesion.
...
- **Feedback loops:** US intervention lifts protest escalation, defection, and collapse probabilities (soft: +5–7%, hard: +15–20%). Economic stress increases protest escalation (+20% at CRITICAL level). Concessions dampen escalation (x0.5) and defection (x0.7). Regional instability raises crackdown probability (+20%). Contested succession triggers rial crash (x1.25) which can escalate economic stress to CRITICAL mid-run.
